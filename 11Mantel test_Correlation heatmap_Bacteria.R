...
d2 <- clone(micro_data)
d2$tax_table <- d2$tax_table[d2$tax_table$Phylum == "p__Acidobacteria", ]
d2$tidy_dataset()
d2$cal_betadiv()

d3 <- clone(micro_data)
d3$tax_table <- d3$tax_table[d3$tax_table$Phylum == "p__Chloroflexi", ]
d3$tidy_dataset()
d3$cal_betadiv()

d4 <- clone(micro_data)
d4$tax_table <- d4$tax_table[d4$tax_table$Phylum == "p__Actinobacteria", ]
d4$tidy_dataset()
d4$cal_betadiv()


d5 <- clone(micro_data)
d5$tax_table <- d5$tax_table[d5$tax_table$Phylum == "p__Rokubacteria", ]
d5$tidy_dataset()
d5$cal_betadiv()
...
x2 <- data.frame(spec = "Acidobacteria", t2$res_mantel) %>% .[, c(1, 3, 6, 8)]
x3 <- data.frame(spec = "Chloroflexis", t3$res_mantel) %>% .[, c(1, 3, 6, 8)]
x4 <- data.frame(spec = "Actinobacteria", t4$res_mantel) %>% .[, c(1, 3, 6, 8)]
x5 <- data.frame(spec = "Rokubacteria", t5$res_mantel) %>% .[, c(1, 3, 6, 8)]
# rename columns
colnames(x1) <- colnames(x2) <- colnames(x3) <- colnames(x4)<- colnames(x5) <- c("spec", "env", "r", "p.value")
...
x5 %<>% dplyr::mutate(rd = cut(r, breaks = c(-Inf, 0.3, 0.6, Inf), labels = c("< 0.3", "0.3 - 0.6", ">= 0.6")),
                      pd = cut(p.value, breaks = c(-Inf, 0.01, 0.05, Inf), labels = c("< 0.01", "0.01 - 0.05", ">= 0.05")))
# cobine two tables
plot_table <- rbind(x1, x2,x3,x4,x5)
